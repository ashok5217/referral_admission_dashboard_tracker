...
#new_ix = pd.Index(range(daily_applications['day'].max()+1))

#output = df2.set_index(['user_id', 'quiz_id'])
#df1=df1.reindex(new_ix, fill_value=0).sort_index().transpose()
#df1
...
ws = client.open_by_url("https://docs.google.com/spreadsheets/d/14m4ntZrZ_ABpxLQh-fJBhkVUECWeayzWsUhA2AzTN48/edit#gid=1122234254").worksheet("ft_raw")


# In[47]:


set_with_dataframe(ws,df2,row = 4,col = 1,include_index = True)
...
application['masai_foundation_program'].unique()
application = application[application['masai_foundation_program']=='Yes']
...
set_with_dataframe(ws,df2,row = 20,col = 1,include_index = True)
...
df_w
    ...
    # In[44]:
final = pd.merge(df_d,df_w,left_index = True,right_index = True,how = 'inner')
...
populate("https://docs.google.com/spreadsheets/d/14m4ntZrZ_ABpxLQh-fJBhkVUECWeayzWsUhA2AzTN48/edit#gid=1122234254","ft_raw_journey",final.astype(int),row = 22,col = 2)
...
populate("https://docs.google.com/spreadsheets/d/14m4ntZrZ_ABpxLQh-fJBhkVUECWeayzWsUhA2AzTN48/edit#gid=1122234254","ft_raw_journey",final.astype(int),row = 4,col = 2)