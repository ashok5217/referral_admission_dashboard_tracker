...
populate("https://docs.google.com/spreadsheets/d/14m4ntZrZ_ABpxLQh-fJBhkVUECWeayzWsUhA2AzTN48/edit#gid=356729337","da_pt_raw",final.astype(int),row = 22,col = 2)
...
#application=pd.read_sql("select a.profile_slug,a.status,a.funnel_entry_date,a.created_at,a.launch_form,a.onboarding_session_one_time_link , b.name,b.email,a.masai_foundation_program,batch.application_start_date,batch.start_date as batch_start_date from application a  left join user b on a.profile_slug=b.slug  left join batch_campus on a.batch_campus_id = batch_campus.id left join batch on batch_campus.batch_id = batch.id where  batch_campus.batch_id = 12 and batch_campus.course_specialization_id = 3 and a.status not in ('INELIGIBLE','PROFILE_UNFILLED') and b.referred_by is not null",con)
...
populate("https://docs.google.com/spreadsheets/d/14m4ntZrZ_ABpxLQh-fJBhkVUECWeayzWsUhA2AzTN48/edit#gid=356729337","da_pt_raw",df2.astype(int),row = 2,col = 2)